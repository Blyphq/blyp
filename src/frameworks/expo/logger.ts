  ...
  try {
    ...
    if (response.ok) {
      ...
      if (
        ...
        errorOnce(
          `missing-http-server-config:${config.connector.name}`,
          `[blyp/expo] HTTP target "${config.connector.name}" was requested but not configured on the server. Continuing without HTTP forwarding.`
        );
      }
